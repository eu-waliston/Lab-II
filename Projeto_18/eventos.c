...
void executarEvento(FilaEventos* fila) {
    ...
    // Seleciona o evento da fila de maior prioridade disponível
    if (fila->Alta) {
        evento = fila->Alta;
    ...
    if (evento) {
        ...
        // Cria novo valor com timestamp
        Valor* novoValor = (Valor*) malloc(sizeof(Valor));
        if (!novoValor) {
            printf("Erro ao alocar memoria para novo valor\n");
            free(evento);
            return;
        }
        ...
        novoValor->valor = evento->valor;
        novoValor->prox = NULL;

        // Insere o valor na lista do sensor/atuador
        if (evento->sensor->valores == NULL) {
            evento->sensor->valores = novoValor;
        } else {
            Valor* aux = evento->sensor->valores;
            while (aux->prox) aux = aux->prox;
            aux->prox = novoValor;
        }

        printf("Valor registrado com sucesso: %s\n", buffer);
        printf("=======================================\n");
